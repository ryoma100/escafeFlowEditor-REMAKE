...
export function containsRect(rect: Rectangle, point: Point): boolean {
  return !(point.x < rect.x || rect.x + rect.width < point.x || point.y < rect.y || rect.y + rect.height < point.y);
}
...
export function minLengthOfPointToRect(point: Point, rect: Rectangle): number {
  ...
  radius = Math.min(radius, pointLength(point, { x: rect.x + rect.width, y: rect.y + rect.height }));
